...
const TOKEN_KEY = testSlot ? `bedazzled3d_token_slot${testSlot}` : 'bedazzled3d_token';
const ROOM_KEY = testSlot ? `bedazzled3d_room_slot${testSlot}` : 'bedazzled3d_room';
...
document.getElementById('btn-confirm-setup').addEventListener('click', () => {
  ...
  if (pendingIntent === 'create') {
    socket.emit('createRoom', { name, avatar: selectedAvatar, token: myToken, board28: true });
...
// Zentrale Steuerung für Eingabefeld + Status-Label + Button (siehe client.js für die
// ausführliche Begründung - identische Logik, damit 2D und 3D konsistent bleiben).
...
// ---------- Live-Anbindung des 3D-Bretts (iframe) ----------
// Emoji -> avatarKey/Farbe, passend zur Registry in avatars/registry.js
const AVATAR_EMOJI_TO_KEY = { '💎': 'diamond', '🎭': 'mask', '🔮': 'crystalball', '🃏': 'joker', '👑': 'crown', '⭐': 'star' };
...
function sendToBoard3D(msg) {
  const frame = document.getElementById('board-3d-frame');
  if (!frame || !frame.contentWindow) return;
  if (!board3dReady) { board3dPendingMessages.push(msg); return; }
  frame.contentWindow.postMessage(msg, '*');
}
// Ersetzt die alte 2D-Board-Darstellung (renderBoardLarge) - schickt echte Spielernamen,
// Avatare und Positionen ans 3D-Brett-iframe, inkl. Zug-Animation (alt -> neu je Spieler).
function sync3DBoard(players, fromPositions, animate, onComplete) {
  const playersPayload = players.map(p => {
    ...
    return {
      ...
      name: p.name,
      avatarKey: key,
      color: AVATAR_KEY_COLORS[key] || '#D5A1FB',
      position: animate ? (fromPositions[p.id] ?? p.position) : p.position,
    };
  });
...
socket.on('state', (state) => {
  ...
  if (state.phase === 'board') {
    ...
    } else if (state.gameOver && winnerOverlayShownFor !== state.code + state.gameOver.winnerName) {
      // Falls gameOver erst NACH dem ersten Board-Render bekannt wird (Sicherheitsnetz)
      winnerOverlayShownFor = state.code + state.gameOver.winnerName;
      showWinnerOverlay(state.gameOver.winnerName, state.gameOver.awards);
    }
    showScreen('board');
